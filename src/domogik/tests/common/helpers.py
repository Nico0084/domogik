...
def delete_configuration(type, name, host):
    ...
    if result:
        ...
        if 'status' in data:
            if not data['status']:
                print(result.get())
                raise RuntimeError("DbMgr did not return status true on a config.set for {0}-{1}.{2}".format(type, name, host))
            else:
                return True
        else:
            print(result.get())
            raise RuntimeError("DbMgr did not return a status on a config.set for {0}-{1}.{2}".format(type, name, host))
    else:
...
def check_config(type, name, host, key, exp_value):
    ...
    if result:
        ...
        if 'status' in data:
            if not data['status']:
                print(result.get())
                raise RuntimeError("DbMgr did not return status true on a config.set for {0}-{1}.{2} : {3} = {4}".format(type, name, host, key, exp_value))
            else:
                if 'value' in data:
                    ...
                    print(result.get())
                    raise RuntimeError("DbMgr did not return a value on a config.set for {0}-{1}.{2} : {3} = {4}".format(type, name, host, key, exp_value))
        else:
            print(result.get())
            raise RuntimeError("DbMgr did not return a status on a config.set for {0}-{1}.{2} : {3} = {4}".format(type, name, host, key, exp_value))
    else:
        raise RuntimeError("Error while setting configuration for {0}-{1}.{2} : {3} = {4}".format(type, name, host, key, exp_value))

### Devices tools

def get_device_list(type, name, host):
    cli = MQSyncReq(zmq.Context())
    msg = MQMessage()
    ...
    if result:
        ...
        if 'status' in data:
            if not data['status']:
                print(result.get())
                raise RuntimeError("DbMgr did not return status true on a device.get for {0}-{1}.{2}".format(type, name, host))
            else:
                if 'devices' in data:
                    return data['devices']
                else:
                    raise RuntimeError("DbMgr did not return device list on a device.get for {0}-{1}.{2}".format(type, name, host))
    else:
        raise RuntimeError("Error while get devices list for {0}-{1}.{2}".format(type, name, host))
